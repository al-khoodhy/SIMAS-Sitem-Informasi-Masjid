...
const [formData, setFormData] = useState({
    ...
    // Dummy Kategori (Idealnya ditarik dari API /kategori)
    // const kategoriOptions = [
    //     { id: '1', nama: 'Infaq / Sedekah (Masuk)' },
    //     { id: '2', nama: 'Zakat (Masuk)' },
    //     { id: '3', nama: 'Operasional / Listrik (Keluar)' },
    //     { id: '4', nama: 'Pembangunan (Keluar)' },
    //     { id: '5', nama: 'Kegiatan Sosial (Keluar)' },
    // ];

    // Mengambil data saat komponen dimuat ATAU filter tanggal berubah
    useEffect(() => {
        fetchTransactions();
        fetchMasterData();
    ...
    const fetchMasterData = async () => {
        try {
            ...
            setCampaignList(resCam.data.data.filter(c => c.status === 'aktif')); // Hanya tampilkan campaign yg aktif
            
            // Set default kategori_id ke data pertama jika ada
    ...
    const fetchTransactions = async () => {
        ...
        try {
            const params = {};
            // Jika kedua tanggal diisi, kirim sebagai parameter pencarian ke Laravel
                            ...
                            {loading ? (
                                ...
                                transactions.map((trx) => (
                                    <tr key={trx.id} className="hover:bg-gray-50 transition">
                                        <td className="px-6 py-4">{formatTanggal(trx.tanggal_transaksi)}</td>
                                        <td className="px-6 py-4 font-medium text-gray-800">{trx.keterangan}</td>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex justify-center">
                                                {trx.bukti_foto ? (
                                                    ...
                                                    >
                                                        {/* Desain Thumbnail Gambar */}
                                                        <img 
                                                            src={`http://localhost:8000/storage/${trx.bukti_foto}`} 
                                                            alt="Bukti Transaksi" 
                                                            className="w-12 h-12 object-cover rounded-md border border-gray-200 shadow-sm transition-transform duration-200 group-hover:scale-105"
                                                            onError={(e) => { 
                                                    ...
                                                    <span className="text-gray-400 text-xs italic bg-gray-100 px-2 py-1 rounded border border-gray-200">Tanpa Bukti</span>
                                                )}
                                            </div>
                                        </td>
                                    </tr>
